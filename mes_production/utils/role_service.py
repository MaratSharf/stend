...
class RoleService:
    ...
    def create_role(self, name: str, label: str, description: str = None,
        ...
        try:
            ...
            cursor.execute(f'SELECT 1 FROM role_permissions WHERE role = {ph} LIMIT 1', (name,))
            if cursor.fetchone():
                return False, f"Роль '{name}' уже существует", None

            # Если указан родитель, проверить его существование
            if parent_role:
                cursor.execute(f'SELECT 1 FROM role_permissions WHERE role = {ph} LIMIT 1', (parent_role,))
            ...
            if parent_role:
                cursor.execute(
                    f'SELECT permission FROM role_permissions WHERE role = {ph} AND permission != {ph}',
                    (parent_role, '')
                )
    ...
    def delete_role(self, role_name: str, force: bool = False,
        ...
        try:
            ...
            # Проверка существования роли
            cursor.execute(f'SELECT 1 FROM role_permissions WHERE role = {ph} LIMIT 1', (role_name,))
            if not cursor.fetchone():
                return False, f"Роль '{role_name}' не найдена", None

            # Проверка зависимостей - пользователи с этой ролью
            cursor.execute(f'SELECT COUNT(*) as cnt FROM users WHERE role = {ph}', (role_name,))
            ...
            cursor.execute(f'DELETE FROM role_permissions WHERE role = {ph}', (role_name,))
    ...
    def get_role_dependencies(self, role_name: str) -> Tuple[bool, str, Optional[dict]]:
        ...
        try:
            ...
            # Количество прав
            cursor.execute(
                f'SELECT COUNT(*) as cnt FROM role_permissions WHERE role = {ph} AND permission != {ph}',
                (role_name, '')
            )
    ...
    def _log_audit(self, role_name: str, action: str,
        ...
        try:
            ...
            if not self.db.table_exists(conn, 'role_audit_log'):
                return
